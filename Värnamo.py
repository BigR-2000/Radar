...
def radar(df, lijst):
    ...
    with col1:
        options1 = st.radio('Player1', options=lijst)
    with col3:
...
def inlezen_bestanden(lijst):
    for player in lijst:
        ...
        if options == 'Central Midfielders':
            Cmidfield_radar(df_g, df_d, df_a, df_p, df_f, speler)
        if options == 'Attacking Midfielders':
            Amidfield_radar(df_g, df_d, df_a, df_p, df_f, speler)
        if options == 'Wingers':
            Winger_radar(df_g, df_d, df_a, df_p, df_f, speler)
        if options == 'Full-backs':
            wingback_radar(df_g, df_d, df_a, df_p, df_f, speler)
        if options == 'Central Defenders':
            centerback_radar(df_g, df_d, df_a, df_p, df_f, speler)

# Hoofdpagina met informatie/ en de navigatiebar
col1,col2 = st.columns([8, 1.5])
with col1:
    st.subheader('Performance Analysis Tool KAA Gent')
#with col2:
    #st.image("logo.png", width=150)
st.markdown('Description of the app:')
st.markdown("- This tool allows for the comparison of player data across the 23/24 season (League, Cup, and European matches).")
#st.markdown("- Further filter by position within each formation.")
st.markdown("- This objective portrayal of player performance can complement subjective evaluations, providing a more comprehensive view.")
st.markdown("- Data from both WyScout for technical information and SkillCorner for physical data is utilized. Note that the accuracy of WyScout data may not always be 100%; consider this in any analyses made.")
st.markdown("- All data is converted to averages per 90 minutes.")
st.markdown("- Several players are highlighted for each position, including those from KAA Gent itself, other JPL players, as well as prospects for KAA Gent.")
st.markdown("- Players can always be added or removed from the app.")
...
options = st.sidebar.radio('Position', options=['Strikers', 'Wingers', 'Attacking Midfielders', 'Central Midfielders', 'Full-backs', 'Central Defenders'])
...
title_with_icon('⚽️', "Played Matches")
st.markdown("The number of matches for which data was collected per player.")

games = games.set_index('Player')
spelers = games.reset_index()
col1, col2 = st.columns([2.18, 4])
with col1:
    speler = st.multiselect('Players', options= spelers['Player'])
if not speler:
    st.dataframe(games)
else:
...
title_with_icon('📋', "Technical and Physical Data")
'Overview of both the technical and physical data of the selected players (Per 90 minutes).'

col1, col2, col3, col4 = st.columns([1, 1.5, 1.5, 1.5])
spelers = general.reset_index()
options_4 = []
with col1:
    options_2 = st.radio('Data', options=['General', 'Attacking', 'Passing', 'Defending', 'Physical'])
with col2:
    options_3 = st.multiselect('players', options= spelers['Player'])
    if options_2 == 'General':
        options_4 = st.multiselect('Parameters', options= general.columns)
    if options_2 == 'Attacking':
...
st.markdown("- Compare the data of two players via the radar chart.")
st.markdown("- Compare a player with the average data of a player in the top 6 of the JPL + KAA Gent.")
...
st.write("")
st.write("")
#st.markdown(lijst_spelersgroep)
